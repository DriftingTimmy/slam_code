...
//
//        pcl::copyPointCloud(*filtered_cloud_, *filtered_cloud_ptr);
...
//            pcl::PointCloud<pcl::PointXYZ> segmented_cluster;
//            float x_all = 0;
//            float y_all = 0;
//            float z_all = 0;
//            int size = 0;
...
//                segmented_cluster.push_back(filtered_cloud_ptr->points[*pit]);
...
//
//            std::cout << "Centroids for the publish segment is : "
//                      << seg_cloud.centroid << std::endl;
//
//            seg_cloud.size = size;
...
//        segmented_cloud.eigen_value_feature[5] =
//                (e1 * std::log(e1)) + (e2 * std::log(e2)) + (e3 * std::log(e3)) / kEigenEntropyMax;///eigen_entropy
